...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, Q: torch.Tensor, K: torch.Tensor, V: torch.Tensor, mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        #TODO
        q = self.query_layers(Q).view(Q.size(0), -1, self.n_heads, self.d_model).transpose(1, 2)
        k = self.key_layers(K).view(K.size(0), -1, self.n_heads, self.d_model).transpose(1, 2)
        v = self.value_layers(V).view(V.size(0), -1, self.n_heads, self.d_model).transpose(1, 2)
        output, _ = self.attention(q, k, v, mask)
        output = output.transpose(1, 2).contiguous().view(Q.size(0), -1, self.n_heads * self.d_model)
